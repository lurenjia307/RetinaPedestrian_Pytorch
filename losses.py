...
def calc_iou(a, b):
    ...
    mm = torch.unsqueeze((a[:, 2] - a[:, 0]) * (a[:, 3] - a[:, 1]), dim=1)
...
class LossLayer(nn.Module):
    ...
    def forward(self,classifications,bbox_regressions,ldm_regressions,anchors,annotations):
        ...
        for j in range(batch_size):
            ...
            # OHEM
            negative_losses = classification[negative_indices,1] * -1  #classification为LogSoftmax输出，需要x轴翻转，所以乘以-1
            ...
            positive_losses = classification[positive_indices,0] * -1  #classification为LogSoftmax输出，需要x轴翻转，所以乘以-1
            ...
            if focal_loss:
                ...
                focal_weight = targets*torch.where(torch.eq(targets, 1), 1. - torch.exp(classification), torch.exp(classification))
                focal_weight = alpha_factor * torch.pow(focal_weight, gamma)
                bce = -(classification)#+ (1.0 - targets) * torch.log(1.0 - classification))
                #add by yzk
            ...
            if positive_indices.sum() > 0:
                # bbox
                bbox_assigned_annotations = bbox_assigned_annotations[positive_indices, :]  #取正样本对应的anchor box
                ...
                bbox_targets = torch.stack((targets_dx, targets_dy, targets_dw, targets_dh))  #根据annotation数据构造预测数据格式的数据
