...
class TrainDataset(Dataset):
        ...
        for line in lines:
            ...
            if line.startswith('#'):
                if isFirst is True:
                    ...
                    labels.clear()       
                path = line[2:]
                #yzk need change for datasets
                #for face dect
                # path = txt_path.replace('label.txt','images/').replace('train.txt','images/').replace('val.txt','images/') + path
                #for pedestrian dect
                path = path.replace('label.txt', 'images/').replace('train.txt', 'images/').replace('val.txt', 'images/')
                #add by yzk
                ...
                # cv2.rectangle(img,(int(label[0]), int(label[1])), (int(label[0]+label[2]), int(label[1]+label[3])), (255,0,0), 2)
                # cv2.circle(img, (int(label[4]), int(label[5])), 1,  (255, 0, 255), 4)
                # cv2.putText(img, '1', (int(label[4]), int(label[5])), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2,
                #             1)
                # cv2.circle(img, (int(label[7]), int(label[8])), 1, (255, 0, 255), 4)
                # cv2.putText(img, '2', (int(label[7]), int(label[8])), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2,
                #             1)
                # cv2.circle(img, (int(label[10]), int(label[11])), 1, (255, 0, 255), 4)
                # cv2.putText(img, '3', (int(label[10]), int(label[11])), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2,
                #             1)
                # cv2.circle(img, (int(label[13]), int(label[14])), 1, (255, 0, 255), 4)
                # cv2.putText(img, '4', (int(label[13]), int(label[14])), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2,
                #             1)
                # cv2.circle(img, (int(label[16]), int(label[17])), 1, (255, 0, 255), 4)
                # cv2.putText(img, '5', (int(label[16]), int(label[17])), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2,
                #             1)
    ...
    def __getitem__(self,index):
        #change by yzk
        # img = skimage.io.imread(self.imgs_path[index])#,as_gray=True)
        #print(self.imgs_path[index])
        img1 = skimage.io.imread(self.imgs_path[index],as_gray=True).astype(np.float32)
        ...
        for idx, label in enumerate(labels):
            ...
            # yzk disable for pedestain dect
            # annotation[0,4] = label[4]                  # l0_x
            # annotation[0,5] = label[5]                  # l0_y
...
class PadToSquare(object):
    def __call__(self, sample, input_size=640):    
        ...
        padded_img = F.pad(img, pad, "constant", value=0) #yzk  pad img using 0  why not 127
        ...
        for line in lines:
            ...
            if line.startswith('#'):
                if isFirst is True:
                    ...
                    labels.clear()       
                path = line[2:]
                # path = txt_path.replace('label.txt','images/') + path  #yzk  for face dect
                path = path.replace('label.txt', 'images/')   #yzk  for perestrain dect
                self.imgs_path.append(path)            
            else:
                line = line.split(' ')
                label = [float(x) for x in line]
