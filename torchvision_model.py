...
class LandmarkHead(nn.Module):
    def __init__(self,inchannels=512,num_anchors=3):
        ...
        # self.conv1x1 = nn.Conv2d(inchannels,num_anchors*10,kernel_size=(1,1),stride=1,padding=0)  #yzk for face dect
        self.conv1x1 = nn.Conv2d(inchannels, num_anchors * 4, kernel_size=(1, 1), stride=1,padding=0)  # yzk for pedestrain dect

    def forward(self,x):
        out = self.conv1x1(x)
        out = out.permute(0,2,3,1)

        # return out.contiguous().view(out.shape[0], -1, 10) #yzk for face dect
        return out.contiguous().view(out.shape[0], -1, 4)    # yzk for pedestrain dect
